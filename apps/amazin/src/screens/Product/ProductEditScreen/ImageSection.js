...
export default function ImageSection({ hook: [images, setImages] }) {
  ...
  return (
    <>
      <div>
        ...
        {images.map((img, id) => (
          <div className="row img-row" key={id}>
            ...
            />
            <button disabled={uploadState.loading} onClick={handleDeleteImg(id)}>
              <i className="fa danger fa-close"></i>
            </button>
            <button disabled={uploadState.loading} onClick={handleMoveUpImg(id)}>
      ...
      {images.length < MAX_IMAGES && (
        <div>
          Add
          <CustomInput text="New Images" name="images" type="file" multiple onChange={handleAddImgs} />
          Or
          <CustomInput text="Image Link" hook={[imagePreview, setImagePreview]} onKeyUp={handleAddImgLinkOnEnter} />
